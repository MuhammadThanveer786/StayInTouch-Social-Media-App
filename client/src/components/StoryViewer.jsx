...
const renderContent = ()=>{
  switch (viewStory.media_type) {
    case 'image':
      ...
      case 'video':
        return(
          <video onEnded={()=>setViewStory(null)} src={viewStory.media_url} alt="" className='max-w-full max-h-screen ' controls autoPlay/>
        );
      case 'text':
        return(
          <div>
  ...
  return (
    <div className='fixed inset-0 h-screen bg-black bg-opacity-90 z-110 flex items-center justify-center' style={{backgroundColor: viewStory.media_type === 'text' ? viewStory.background_color : '#000000'}}>
       ...
       <div className='absolute top-0 left-0 w-full h-1 bg-gray-700'>
        <div className='h-full bg-white transition-all diration-100 linear' style={{width:`${progress}%`}}>
        </div>
       </div>
       {/* user Info - Top Left */}
      ...
      <div className='text-white font-medium flex items-cneter gap-1.5'>
